...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async loadMoreTemp(dom) {
        // 判断参数是否有值，无值直接返回
        if (!dom) return;
        // 判断是否正在加载中，或加载到最大值，如果是就直接返回
        if (this.isLoading2 || this.data.rows.length >= this.data.total) return;
        // 定义一个判定范围
        const range = 100;
        // 判断滚动条是否滚动到底部，滚动到底部则加载更多数据
        if (dom.scrollHeight - dom.scrollTop - dom.clientHeight < range) {
          this.isLoading2 = true; // 开始加载
          this.page++; // 设置参数
          const res = await this.fetchData(); // 发出请求拿到结果
          // 更新数据
          this.data.total = res.total;
          this.data.rows = this.data.rows.concat(res.rows);
          this.isLoading2 = false; // 结束加载
        }
      }
    },
    mounted() {
      // 函数防抖
      this.loadMore = debounce(this.loadMoreTemp);
      // 在事件总线上监听
      this.$eventListener.$on('scan', this.loadMore);
    },
    destroyed() {
      // 组件销毁时移除事件总线上的监听
      this.$eventListener.$off('scan', this.loadMore);
    }
  };
</script>
